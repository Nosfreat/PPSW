...
void (*ptrTimer0InterruptFunction)(void); 				//stworzenie wskaznika(wskazuje na funkcje bez argumentow)
...
__irq void Timer0IRQHandler(){
	...
	ptrTimer0InterruptFunction();								//wykonanie funkcji ze wskaznika
...
void Timer0Interrupts_Init(unsigned int uiPeriod, void (*ptrInterruptFunction)() ){ // microseconds
	...
	ptrTimer0InterruptFunction = ptrInterruptFunction;		// ustawienie wskaznika
